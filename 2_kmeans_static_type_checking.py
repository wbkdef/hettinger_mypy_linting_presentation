...
def assign_data(centroids: Sequence[Centroid], data: Iterable[Point]) -> Dict[str, Sequence[Point]]:
    'Assign data the closest centroid'
    d : DefaultDict[Centroid, List[Point]] = defaultdict(list)
    for point in data:
        centroid: float = min(centroids, key=partial(dist, point))  # type: ignore
